...
public class CompanyPostController {
    ...
    public ResponseEntity<DataResult<Collection<CompanyPost>>> getAllPageable(@PathVariable Integer page,
                                                                           @PathVariable Integer size) {
        var result = companyPostService.getAllPageable(PageRequest.of(page, size));
        if (!result.isSuccess()) {
            return new ResponseEntity<>(result, HttpStatus.BAD_REQUEST);
        }
        return new ResponseEntity<>(result, HttpStatus.OK);
